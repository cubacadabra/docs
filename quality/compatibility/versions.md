...
| SDK | Runtime/native builder accept `0.3.0`, `0.4.0`, `0.5.0`, and `0.6.0` | Terrain authoring requires `0.4.0`, `0.5.0`, or `0.6.0`; collision, authored gameplay camera, and horizontal bounds require explicit `0.5.0` or `0.6.0`; non-uniform mesh `scale3` requires `0.6.0`. The default creator still emits `0.3.0`; the retired Python builder rejects current native capabilities. |
| Static collision | Manifest format 1, SDK `0.5.0`/`0.6.0` | Reject unknown versions and reject collision data without SDK `0.5.0` or `0.6.0`; the collision format remains separately versioned. |
