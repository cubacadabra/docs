...
The live cooperative path is documented precisely in the [network contract](../../contracts/network.md); the trusted authority seam below remains a prototype.
...
network, persistence, filesystem, or renderer APIs. The builder detects
`src/server.luau`, bundles it as a separate `authority.luau` entry,
...
round timeout. When `src/server.luau` exists, the package builder automatically
bundles it as `authority.luau` and records the entry in the JSON
manifest/package descriptor and file hash list. This adds no binary prefab/scene format: authored world
